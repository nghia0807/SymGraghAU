...
# TODO: chỉnh lại cho đúng thứ tự AU trong DISFA của bạn
AU_NAMES = ["AU1", "AU2", "AU4", "AU5", "AU6", "AU9", "AU12", "AU25"]

AU_NAME_TO_IDX = {name: i for i, name in enumerate(AU_NAMES)}
IDX_TO_AU_NAME = {i: n for n, i in AU_NAME_TO_IDX.items()}

# Emotion xuất hiện trong rule: Happ, Sad, Surp, Fear, Disg
# Ví dụ: nếu head có 7 lớp: [Anger, Disg, Fear, Happ, Sad, Surp, Neutral]
EXPR_NAME_TO_IDX = {
    "Happ": 3,
    "Sad": 4,
    "Surp": 5,
...
class LogicRuleBasePySAT:
    ...
    def _build_cnf_str_for_sample(self, y_a, p_expr=None):
        ...
        for clause in self.cnf_ae:
            # xác định emotion ở cuối clause
            ...
            if emo_name in EXPR_NAME_TO_IDX and not emo_neg and p_expr is not None:
                ...
                # p^uct lớn nhưng không =1
                p_uct = max(self.min_p_uct, min(self.max_p_uct, p_e))
            else:
                # nếu không có p_expr → dùng hằng
                p_uct = 0.8
        ...
        for clause in self.cnf_excl:
            ...
            if name1 in AU_NAME_TO_IDX and name2 in AU_NAME_TO_IDX:
                ...
                if i < len(y_a) and j < len(y_a):
                    # chỉ khi liên quan ít nhất một AU đang on
    ...
    def _build_base_cnf_for_sample(self, y_a, p_expr=None):
        """
        Xây CNF PySAT + cnf_str cho sample (dựa trên _build_cnf_str_for_sample)
        """
    ...
    def sample_satisfying_assignment(self, y_a, p_expr=None):
        ...
        with Minisat22(bootstrap_with=cnf) as solver:
            sat = solver.solve()
            if not sat:
                # fallback: GT AU + 0 cho phần còn lại
                assign_s = {}
...
def train_phase2(
    ...
    acc_disc_meter = AverageMeter()        # NEW
    triplet_sat_meter = AverageMeter()     # NEW
